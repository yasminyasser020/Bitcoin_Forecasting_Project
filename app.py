...
with st.sidebar:
    ...
    model_choice = st.selectbox("Select Algorithm", ["Prophet", "ARIMA"])
...
if uploaded_file is not None:
    ...
    # For dates, we include everything because dates can be strings or numbers
    date_candidates = all_cols

        
    c1, c2 = st.columns(2)
    with c1:
        # User only sees relevant date columns
        col_date = st.selectbox(
            "Identify Timestamp Column", 
            options=date_candidates, 
            index=None, 
            placeholder="Choose date/time column..."
        )
    with c2:
     
        col_price = st.selectbox(
            "Identify Price Column (Close/Open)", 
            options=numeric_cols, 
            index=None, 
            placeholder="Choose numeric price column..."
        )
    if not col_date or not col_price:
        st.warning("Please select both the **Date** and **Price** columns above to proceed.")
        st.stop() # Prevents the rest of the script from running until columns are picked
